...
" set autoread " Re-read a file that changed but was not edited in the buffer

" Enable filetype detection, and the plugin and indent folders
filetype on
" filetype plugin on
" filetype indent on
...
" Change vim's working dir to the current buffer's file's dir
...
" Make window resizing easier by using arrow keys
